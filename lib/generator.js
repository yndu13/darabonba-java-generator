...
class Visitor {
  ...
  async visitPom() {
    ...
    if (!havePom) {
      ...
      dependenciesClass.push('com.aliyun:tea:[1.0.3, 2.0.0)');
  ...
  getSubFieldClassName(className, hasModel) {
    if (className.indexOf('.') > 0) {
      ...
      name += _subModelName(className, this.conflictModelNameMap, this.allModleNameMap);
      return name;
    }
    return className;
  }
  ...
  visitType(ast, isSubType = false) {
    if (ast.type === 'map') {
      ...
      let subModelName = '';
      if (rest.length > 1) {
        subModelName = `.${_subModelName(pathName, this.conflictModelNameMap, this.allModleNameMap)}`;
      }
      ...
      if (this.conflictModels.get(checkModel)) {
    ...
    if (value.fieldType === 'array') {
      ...
      if (value.fieldItemType.tag === 8) {
        ...
        if (node.fieldValue.itemType) {
          this.emit(_subModelName(node.fieldValue.itemType, this.conflictModelNameMap, this.allModleNameMap));
        } else {
      ...
      this.emit(_subModelName([modelName, _name(node.fieldName)].join('.'), this.conflictModelNameMap, this.allModleNameMap));
    }
  }
  ...
  createGetSetMethod(ast, level, modelName) {
    ...
    for (let i = 0; i < ast.nodes.length; i++) {
      ...
      this.emit(`public ${_subModelName(modelName, this.conflictModelNameMap, this.allModleNameMap)} set`, level);
  ...
  eachModel(ast, level) {
    ...
    this.emit(`public class ${_subModelName(modelName, this.conflictModelNameMap, this.allModleNameMap)} extends TeaModel {\n`, level);
  ...
  eachSubModel(ast, level) {
    ...
    const modelName = _name(ast.modelName);
    this.visitAnnotation(ast.annotation, level);
    this.emit(`public static class ${_subModelName(modelName, this.conflictModelNameMap, this.allModleNameMap)} extends TeaModel {\n`, level);
    this.visitModelBody(ast.modelBody, level + 1, modelName);
    this.visitBuildMethod(ast, level + 1);
    this.createGetSetMethod(ast.modelBody, level + 1, modelName);
    this.emit('}\n\n', level);
  ...
  visitImport() {
    for (var item of this.usedExternModel.entries()) {
      this.emit(`import ${this.imports[item[0]].package}.*;\n`);
      if (item[1].size > 0) {
        this.emit(`import ${this.imports[item[0]].package}.models.*;\n`);
      }
    }
  }
